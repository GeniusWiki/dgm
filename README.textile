...
Add files to DGM repository, file could be with relative or absolute path.
*dgm add [-init] nagios3 [ /etc/init.d/tomcat.sh  ~/nginx.conf]* 
  -init: Used for first time to add file, after add, there is git commit immediately. Warning, commit will for all staging changes not this new added file.
  
Check DGM repository and git status 
*dgm status [-a]*
	-a: List all files in DGM repository and DGM configuration information.
